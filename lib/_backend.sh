...
backend_db_create() {
  ...
  docker run --name flowdeskpro-redis \
                -e TZ="America/Sao_Paulo" \
                -p 6379:6379 \
                --restart=always \
...
# Limitar Uso do flowdeskpro Usuario e Conexões
USER_LIMIT=99
...
whatsappweb_update() {
  print_banner
  printf "${WHITE} 💻 Atualizando o whatsapp.js e removendo cache sem perder dados...${GRAY_LIGHT}"
  printf "\n\n"
